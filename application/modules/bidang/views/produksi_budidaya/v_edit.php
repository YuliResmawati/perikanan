<div class="row mt-4">
    ...
    <?= form_open($uri_mod.'/AjaxSaveEdit/'.encrypt_url($id, $id_key), 'id="formAjax" class="form"') ?>
        <input type="hidden" class="pbe-token-response" name="pbe-token-response">
        <div class="form-group row">
            <label for="media" class="col-md-2 col-form-label">Pilih Media <?= label_required() ?></label>
            <div class="col-md-10">
                <select class="form-control select2" name="media" id="media">
                    <option selected disable>Pilih Jenis Media</option>
                    <?php $no = 1; foreach($media as $row): ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function(e) {
        ...
        if (data != false) {
            let produksi = formatRupiah(data.data.jumlah_produksi);

            $('select[name="media"]').val(data.data.media_budidaya).change();
    ...
    $('#submit-btn').click(function(e) {
        ...
        grecaptcha.ready(function() {
            grecaptcha.execute('<?php echo RECAPTCHA_SITE_KEY; ?>', {action: 'submit'}).then(function(token) {
                document.querySelector('.pbe-token-response').value = token;
                $('#formAjax').submit()
            });
        });
    });
