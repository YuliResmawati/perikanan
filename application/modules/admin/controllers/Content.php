...
class Content extends Backend_Controller {
    ...
    public function __construct()
	{
		...
		$this->_init();
		$this->data['uri_mod'] = 'admin/content';
        $this->id_key = $this->private_key;
        $this->breadcrumbs->push('Content', 'admin/content');
        $this->load->model([
	...
	{
        $this->data['add_button_link'] = base_url('admin/content/add');
        $this->data['page_title'] = "Data Content";
        $this->data['page_description'] = "Halaman Daftar Data Content.";
        $this->data['card'] = "true";
        $this->data['breadcrumbs'] = $this->breadcrumbs->show();
        $this->data['id_key'] = $this->id_key;
        $this->data['kategori'] = $this->m_kategori_content->where(['id !=' => '1'])->findAll();
        
		$this->load->view('content/v_index', $this->data);
    }

    public function add()
    {
        $this->breadcrumbs->push('Tambah', 'admin/content/add');
        $this->data['page_title'] = "Tambah Data Content";
        $this->data['page_description'] = "Halaman Tambah Data Content.";
        $this->data['card'] = "true";
        $this->data['breadcrumbs'] = $this->breadcrumbs->show();
        $this->data['kategori'] = $this->m_kategori_content->where(['id !=' => '1'])->findAll();
        $this->data['id_key'] = $this->id_key;

        $this->load->view('content/v_add', $this->data);
    }
    ...
    {        
		...
		if ($id == FALSE) {
			$this->load->view('errors/html/error_bootbox.php', array('message' => 'ID yang tertera tidak terdaftar', 'redirect_link' => base_url('admin/content')));
        }

        $this->breadcrumbs->push('Edit', 'admin/content/edit');
        $this->data['page_title'] = "Edit Data Konten";
        $this->data['page_description'] = "Halaman Edit Data Konten.";
        $this->data['card'] = "true";
        $this->data['breadcrumbs'] = $this->breadcrumbs->show();
        $this->data['kategori'] = $this->m_kategori_content->where(['id !=' => '1'])->findAll();
        $this->data['id_key'] = $this->id_key;
        ...
        $this->load->view('content/v_edit', $this->data);
    }

    private function upload_thumbnail($id, $type = '')
    {
        $this->result = array('status' => TRUE, 'message' => NULL);
        $file_name = 'dpkk_thumbnail_content'.'_'.date('ymdhis');

        if ($this->result['status'] == TRUE) {
            $config['upload_path'] = $this->data['image_path'].'content/';
            ...
            if (!$this->upload->do_upload('image')) {
                ...
                $this->result = array(
                    'status' => true,
                    'message' => 'Upload gambar content berhasil',
    ...
    public function AjaxGet($id = NULL)
    {
        ...
        if ($id == FALSE) {
            ...
            $edit_link = 'admin/content/edit/'; 
            $response = $this->m_content->get_all_content()->datatables();

            if (decrypt_url($this->input->post('filter_kategori'), $this->id_key) == FALSE) {
                $response->where('kategori_konten_id', 0);
            } else {
                if (decrypt_url($this->input->post('filter_kategori'), $this->id_key) !== 'ALL') {
                    $response->where('kategori_konten_id', decrypt_url($this->input->post('filter_kategori'), $this->id_key));
                }
            }

            $response->where('kategori_konten_id !=', '1');
            ...
            $response->edit_column('link', '$1', "btn_content_view(slug, nama_kategori)");
            ...
            if ($this->return !== FALSE) {
                ...
                $this->return->berkas = str_files_images('content/', $this->return->berkas);
    ...
    {
        $this->output->unset_template();
        $captcha_score = get_recapture_score($this->input->post('c-token-response'));  
        ...
        if ($captcha_score < RECAPTCHA_ACCEPTABLE_SPAM_SCORE) {
            ...
            $this->result = array('status' => TRUE, 'message' => NULL);
            $id = decrypt_url($id, $this->id_key);
            $kategori_id = decrypt_url($this->input->post('kategori'), $this->id_key);
            $check_data = $this->m_content->find($id);

            $this->form_validation->set_rules('kategori', 'Kategori Content', 'required');
            ...
            if ($this->form_validation->run() == TRUE) {    
                if ($id == FALSE) {
                    ...
                    $this->m_content->push_to_data('status', '1')
                        ...
                        ->push_to_data('kategori_konten_id', $kategori_id)
                        ->push_to_data('hits', '0');
                    ...
                    $this->m_content->push_to_data('kategori_konten_id', $kategori_id);
                }
