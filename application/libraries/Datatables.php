  ...
  class Datatables
  {
          ...
          if ($sSearch != '') {
              for ($i = 0; $i < count($mColArray); $i++) {
                  if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns)) {
                      if ($this->check_cType()) {
                        $col_name = $this->select[$mColArray[$i]['data']];
                        $col_name = "\"".str_replace(".", "\".\"",$col_name)."\"";
                        if($this->ci->db->dbdriver == 'postgre') {
                            $sWhere .= "lower(" . $col_name . "::text) LIKE '%" . strtolower($sSearch) . "%' OR ";
                        }else{
                            $sWhere .= "LOWER(" . $col_name . ") LIKE '%" . strtolower($sSearch) . "%' OR ";
                        }
                      } else {
                        if($this->ci->db->dbdriver == 'postgre') {
                            $sWhere .= "lower(" . $this->select[$this->columns[$i]] . "::text) LIKE '%" . $sSearch . "%' OR ";
                        }else{
      ...
      */
      private function produce_output($output, $charset)
      {
          ...
          if ($output == 'json') {
            ...
            {
                if(!empty($sOutput['data'])) {
                    $sOutput['data'] = xss_escape($sOutput['data']);
                }
      ...
      */
      private function get_total_results($filtering = false)
      {
          ...
          $countingsql = "SELECT COUNT(*) AS count FROM (" . $subquery . ") SqueryAux";
