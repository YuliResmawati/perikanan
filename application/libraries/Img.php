...
class Img
{
	 ...
	 * @return string
	 */
	public function rimg($source, $params, $newconfig=NULL, $no_cache = FALSE)
	{
		if ($newconfig) {
			$this->ci->config->config['img']['base_path'] = $newconfig['base_path'];
			$this->ci->config->config['img']['base_url']  = $newconfig['base_url'];
		}
		...
		if ($src['width'] === $dst['width'] && $src['height'] === $dst['height'])
		{
			...
			if ($params['return']) {
				...
				return '<img src="'.site_url().''.base_url($temp['dirname'] . "/" . basename($dst['file'])) . "\" width=\"{$dst['width']}\" height=\"{$dst['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
			}
		...
		if (file_exists($dst['file']) AND $params['nocache'] !== TRUE)
		{
			...
			if ($params['return']) {
				...
				return "<img src=\"".site_url()."{$this->ci->config->config['img']['base_url']}{$dir}/" . basename($dst['file']) . "\" width=\"{$params['width']}\" height=\"{$params['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
			}
		...
		if ($params['return'] == TRUE) {
			...
			return "<img src=\"".site_url()."{$this->ci->config->config['img']['base_url']}{$dir}/" . basename($dst['file']) . "\" width=\"{$dst['width']}\" height=\"{$dst['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
		}

	}

	public function rimg_url() {
		# code...
	}
	...
	public function rimg_f($source, $params, $newconfig=NULL, $no_cache = FALSE)
	{
		if ($newconfig) {
			$this->ci->config->config['img']['base_path'] = $newconfig['base_path'];
			$this->ci->config->config['img']['base_url']  = $newconfig['base_url'];
		...
		if (file_exists($dst['file']) AND $params['nocache'] !== TRUE)
		{
			...
			if ($params['return']) {
				...
				return "<img src=\"".site_url()."{$this->ci->config->config['img']['base_url']}{$dir}/" . basename($dst['file']) . "\"  alt=\"{$params['alt']}\" {$extra_parameters}/>";
			}
