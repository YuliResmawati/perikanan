...
class MY_Form_validation extends CI_Form_validation {
	...
	public function edit_unique($str, $field)
    {
        ...
        $where = array($field => $str, 'id !=' => $id, 'deleted' => '0');
    ...
    public function db_unique($str, $field)
    {
        ...
        $where = array($field => $str, 'deleted' => '0');
    ...
    public function password_unique($str, $field)
    {
        ...
        if (!empty($nip)) {
            ...
            $this->CI->form_validation->set_message('password_unique', "Bidang Nama Pengguna untuk bidang {field} diperlukan.");
            $return = FALSE;
        }
    ...
    public function password_check_field($str, $field)
    {
        $return = true;
        sscanf($field, '%[^.].%[^.].%[^.].%[^.]', $table, $field, $id, $field_label);
        if (!empty($id)) {
            $this->CI->form_validation->set_message('password_check_field', "{field} yang anda inputkan harus berbeda dengan $field_label.");
            ...
            if (isset($this->CI->db)) {
                $query = $this->CI->db
                    ...
                    ->or_where("deleted", '0')
                    ->group_end()
                    ->get_where($table, array('id =' => $id, 'status' => '1'));
