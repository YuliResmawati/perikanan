...
class Breadcrumbs {
	...
	*
    */
	protected $template_name = 'breadcrumb_default';
	...
	/**
	 * Reinitiate other config for breadcrumbs
	 *
	 * @access	public
	 * @param	string $config
	 * @return	void
	...
	function set_config($config)
	{
		...
		$this->divider = $this->ci->config->item('divider', $config);
		...
		$this->crumb_anchor_attr = $this->ci->config->item('crumb_anchor_attr', $config);
	}
		...
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end($keys) == $key) {
					...
					$output .= $this->crumb_open.'<a '. $this->crumb_anchor_attr .' href="' . $crumb['href'] . '">' . $crumb['page'] . '</a> '.$this->crumb_divider.$this->crumb_close;
				}
			}
