...
class MY_Model extends CI_Model{
	...
	function __construct() {
        ...
        $this->_user_id = $this->session->userdata($this->_user_session);
	}
    ...
    public function datatables($generate = false) {
        ...
        if ($generate == true) {
            ...
            if ($this->_fields_toshow) {
                if (is_bool(array_search($this->_table.'.'.$this->_primary_key, $this->_fields_toshow))) {
            ...
            if ($this->_join) {
                foreach ($this->_join as $row) {
                    if (!empty($row->join_type)) {
                        $this->datatables->join($row->foreign_table, $row->foreign_key, $row->join_type);
                    } else {
                        $this->datatables->join($row->foreign_table, $row->foreign_key);
                    }
                }
            }
            ...
            if ($this->_softdelete) {
                $this->db->group_start();
                $this->datatables->where($this->_table.'.'.'deleted =', null);
                $this->datatables->or_where($this->_table.'.'.'deleted !=', '1');
                $this->db->group_end();
            }
